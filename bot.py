...
status = ('^help for commands', 'Updated V5', 'Creator: Tron#2819', "Virus: {} √".format(len(client.servers)))
...
async def help(ctx):
    ...
    embed.add_field(name='^invite', value ='***Invite Me To You Servers!***', inline=False)
...
@client.command(pass_context=True)
async def raiterdm(ctx, member : discord.Member, *, content: str):
    await client.send_message(member, content)
...
async def invite():
  	...
  	await client.whisper("Add me with this link https://discordapp.com/oauth2/authorize?client_id=548058363846721536&permissions=8&redirect_uri=https%3A%2F%2Fwww.privategt.com&response_type=code&scope=identify%20bot%20messages.read")

@client.command(pass_context=True)
async def announce(ctx, type):
	await client.say("@everyone ")
	embed = discord.Embed(Colour = discord.Colour.orange())
	embed.add_field(name ='Announcement', value ='***'f'{type}***', inline=False)
	await client.say(embed=embed)
